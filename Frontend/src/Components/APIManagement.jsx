...
function APIManagement() {
    ...
    return (
        <>
            <div className="bg-white shadow-md p-4 flex justify-end items-center rounded-t-lg max-w-3xl mx-auto relative">
                <div className="relative">
                    <button
                        onClick={() => setShowPopup(!showPopup)}
                        className="flex items-center justify-center w-10 h-10 bg-gray-100 rounded-full hover:bg-gray-200 transition duration-200 cursor-pointer"
                    >
                        <User />
                    </button>
                    ...
                    {showPopup && (
                        <div className="absolute right-0 mt-2 w-60 bg-white border rounded-xl shadow-lg p-4 z-50">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <div>
                                    <p className="text-gray-800 font-semibold text-sm">Hello, {currentUser?.firstName?.toUpperCase()}!</p>
                            ...
                            <button
                                ...
                                className="w-full bg-red-500 hover:bg-red-600 text-white py-2 rounded-lg transition duration-200 text-sm font-semibold cursor-pointer"
                            >
                                Logout
                            </button>
                            <hr className="my-2 border-gray-200" />
            ...
            <div className="max-w-3xl mx-auto p-6 bg-gray-50 rounded-lg shadow-md mt-8">
                ...
                <form
                    ...
                    className="flex flex-col gap-4 mb-8 bg-white p-6 rounded-lg shadow-sm"
                >
                    <input
                        type="text"
                        name="url"
                    ...
                    <div className="relative w-full">
                        <input
                            ...
                            onChange={handleChange}
                            required
                            className="w-full border border-gray-300 rounded px-4 py-2 pr-16 focus:outline-none focus:ring-2 focus:ring-blue-400"
                        />
                        <button
                            type="button"
                            onClick={() => setShowPassword(!showPassword)}
                            className="absolute right-2 top-1/2 transform -translate-y-1/2 text-gray-600 hover:text-gray-900 font-semibold cursor-pointer"
                        >
                    ...
                    <textarea
                        ...
                        onChange={handleChange}
                        className="border border-gray-300 rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400 resize-y"
                    />
                    {/* <label className="w-full border border-gray-300 rounded px-4 py-2 flex items-center justify-between cursor-pointer focus-within:ring-2 focus-within:ring-blue-400">
                        ...
                        <input
                            ...
                            onChange={handleChange}
                            className="hidden"
                            onChange={(e) => setFormData(prev => ({ ...prev, fileUpload: e.target.fileUpload[0] }))}
                            required
                        />
                    </label> */}
                    ...
                    <button
                        type="submit"
                        className="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded transition duration-200 cursor-pointer"
                    >
                ...
                {passwords.length > 1 ? (<div className="flex justify-center mb-6">
                    <button
                        onClick={handleDelete}
                        className="bg-red-500 hover:bg-red-600 text-white font-semibold py-2 px-4 rounded transition duration-200 cursor-pointer"
                    >
                ...
                <div className="grid gap-4">
                    {passwords.length > 0 ? (
                        passwords.map((item) => (
                            <div
                                key={item._id}
                                className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition duration-200 flex flex-col md:flex-row md:justify-between md:items-center gap-4"
                            >
                                <div className="flex-1">
                                    ...
                                    <h2 className="font-semibold text-lg space-x-2">
                                        Password: <span className="font-normal select-none" onMouseEnter={() => alert("You can only use the copy button")}>{"•".repeat(item.password.length)}</span>
                                        <button onClick={() => {
                                            navigator.clipboard.writeText(item.password);
                                            toast.success("Password copied!");
                                        }} className="bg-blue-500 hover:bg-blue-600 text-white px-2 py-1 rounded-md text-xs font-semibold transition duration-200 cursor-pointer"> Copy </button>
                                    </h2>
                                    <h2 className="font-semibold text-lg mb-1">
                                        Description: <span className="font-normal">{item.description ? item.description : "N/A"}</span>
                                    </h2>
                                </div>

                                <div className="flex gap-2 mt-4 md:mt-0">
                                    <button
                                        onClick={() => handleUpdateById(item._id)}
                                        className="bg-green-400 hover:bg-green-600 text-white font-semibold py-2 px-4 rounded transition duration-200 cursor-pointer"
                                    >
                                        Update
                                    </button>
